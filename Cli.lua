...
function is_JoinChannel(msg)
	...
	if (not Joinchanel.ok or Joinchanel.result.status == "left" or Joinchanel.result.status == "kicked") and not is_sudo(msg) then
		SendMetion(msg.chat_id,msg.sender_user_id, msg.id,'💥️ کاربر : '..msg.sender_user_id..' شما ابتدا باید در کانال زیر عضو شوید و سپس مجدد دستور خود را ارسال کنید.\nکانال ما :\n>> '..Channel..' <<', 12,string.len(msg.sender_user_id))
		return false
	end
	 return true
end
...
sendText(msg.chat_id,0,"💥️شارژ  "..msg.chat_id.."`💥️ این گروه به اتمام رسیده است لطفا به مدیر ربات مراجعه کنید.`\n▪️➖▪️➖▪️➖▪️➖▪️\n🌐 کانال بیوگرافے :\n"..check_markdown(UserSudo).."\n🌐 کانال پشتیبانے ربات:\n "..check_markdown(Channel).."","md")
local Link = redis:get('Link:'..msg.chat_id) or 'ثبت نشده'
local textt =[[ `.شارز گروه زیر به اتمام رسیده است 💥️`
...
`💥️ 1 ماهه8هزار تومان` 
`💥️ 2 ماهه15هزار تومان`
`💥️ 3 ماهه20هزار تومان`
`💥️ 4 ماهه25هزار تومان`

🔹 نکته مهم :

`➖توجه داشته باشید بدلیل سرعت و امکانات بالا ربات دائمی نداریم`
...
redis:setex("ExpireData:"..msg.chat_id,day,true)
 ...
 redis:sadd("group:",msg.chat_id)
if redis:get('CheckBot:'..msg.chat_id) then
local text = '`💥️ Group '..Company.title..' Is AlReady Added to Management List. `\n\n 🌐 '..check_markdown(Channel)..''
 sendText(msg.chat_id, msg.id,text,'md')
else
local text = '`💥️ Group '..Company.title..' Added to Management List. `\n\n 🌐 '..check_markdown(Channel)..''
local Hash = 'StatsGpByName'..msg.chat_id
local ChatTitle = Company.title
redis:set(Hash,ChatTitle)
print('💥️ New Group\nChat name : '..Company.title..'\nChat ID : '..msg.chat_id..'\nBy : '..msg.sender_user_id)
local textlogs =[[
💥️ New Group Added to Management List‼️

💥️ Group Informations❕:

💥️ Group Name ]]..Company.title..[[

💥️ Group Id : ]]..msg.chat_id..[[

💥️ By : ]]..msg.sender_user_id..[[
...
io.popen("rm -rf ~/.telegram-bot/main/files/videos/*")
io.popen("rm -rf ~/.telegram-bot/main/files/voice/*")
sendText(msg.chat_id,msg.id,'`SuperHero Bot Successfully Reloaded.`\n\n🌐 '..check_markdown(Channel)..'','md')
end 
if matches == 'حذف' then
local function GetName(MaTaDoR, Company)
...
redis:del('CheckExpire:'..msg.chat_id)
 if not redis:get('CheckBot:'..msg.chat_id) then
local text = '`💥️ Group  '..Company.title..' Is Note Added to Management List.`'
 sendText(msg.chat_id, msg.id,text,'md')
else
local text = '`💥️ Group  '..Company.title..'  Removed From Management List.`'
local Hash = 'StatsGpByName'..msg.chat_id
redis:del(Hash)
 sendText(msg.chat_id, msg.id,text,'md')
 redis:del('CheckBot:'..msg.chat_id) 
...
redis:set("ExpireData:"..msg.chat_id,true)
sendText(msg.chat_id ,msg.id,"`💥️ Group Charged Unlimited.`",'md')
if redis:get('CheckExpire:'..msg.chat_id) then
redis:set('CheckExpire:'..msg.chat_id,true)
end
end
...
sendDocument(Sudoid, 0, 0, 1, nil, './Config.lua', dl_cb, nil)
sendDocument(Sudoid, 0, 0, 1, nil, './Api.lua', dl_cb, nil)
sendText(msg.chat_id, msg.id,'`💥️ The Latest Version of The Source Was Sent To The Developer of The Robot.`','md')
end
...
if matches == 'پیکربندی' and is_JoinChannel(msg) then
...
sendText(msg.chat_id,msg.id,"`💥️ All Group Managers were Recognized.`\n\n🌐"..check_markdown(UserSudo).."", "md")
end
getChannelMembers(msg.chat_id,'Administrators',0,limit,GetMod)
end
...
sendText(msg.chat_id, msg.id,'💥️ تمام کاربران دیلیت اکانتی از گروه حذف شدند.' ,'md')
end
tdbot_function ({_= "getChannelMembers",channel_id = getChatId(msg.chat_id).id,offset = 0,limit= 1000}, list, nil)
end
if (matches == 'clean msgs' or matches == 'پاکسازی همه') and is_JoinChannel(msg) then
...
redis:sadd('ModList:'..msg.chat_id,Company.sender_user_id)
local user = Company.sender_user_id
SendMetion(msg.chat_id,user, msg.id, '💥️ User : '..user..' Promoted.', 10,string.len(user))
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id), PromoteByReply)  
...
function DemoteByReply(MaTaDoR,Company)
redis:srem('ModList:'..msg.chat_id,Company.sender_user_id)
SendMetion(msg.chat_id,Company.sender_user_id, msg.id, '💥️ User : '..Company.sender_user_id..' Demoted.', 10,string.len(Company.sender_user_id))
end
if tonumber(msg.reply_to_message_id) == 0 then
else
getMessage(msg.chat_id, tonumber(msg.reply_to_message_id),DemoteByReply)  
...
if matches == 'قفل گروه' and is_JoinChannel(msg) then
redis:set('MuteAll:'..msg.chat_id,true)
sendText(msg.chat_id, msg.id,'💥️ قفل همه فعال شد.' ,'md')
end
if matches == 'lock all' and is_JoinChannel(msg) then
redis:set('MuteAll:'..msg.chat_id,true)
sendText(msg.chat_id, msg.id,'💥️ Lock All Has Been Enable.' ,'md')
end
...
if matches == 'بازکردن گروه' and is_JoinChannel(msg) then
redis:del('MuteAll:'..msg.chat_id)
local mutes =  redis:smembers('Mutes:'..msg.chat_id)
for k,v in pairs(mutes) do
redis:srem('MuteList:'..msg.chat_id,v)
...
sendText(msg.chat_id, msg.id,'💥️ قفل همه غیرفعال شد' ,'md')
end
if matches == 'unlock all' and is_JoinChannel(msg) then
redis:del('MuteAll:'..msg.chat_id)
local mutes =  redis:smembers('Mutes:'..msg.chat_id)
for k,v in pairs(mutes) do
redis:srem('MuteList:'..msg.chat_id,v)
...
sendText(msg.chat_id, msg.id,'💥️ Lock All Has Been Disable.' ,'md')
end
...
if (status[2] == 'del' or status[2] == 'حذف پیام') and is_JoinChannel(msg) then
...
local user = Company.sender_user_id
if redis:sismember('OwnerList:'..msg.chat_id,user) then
SendMetion(msg.chat_id,Company.sender_user_id, msg.id, '💥️ The User:  '..Company.sender_user_id..' is AlReady Added In the List of Owners of The Group ', 10,string.len(Company.sender_user_id))
else
SendMetion(msg.chat_id,Company.sender_user_id, msg.id, '💥️ The User:  '..Company.sender_user_id..' Has Been Added to the List of Owners of The Group. ', 10,string.len(Company.sender_user_id))
redis:sadd('OwnerList:'..msg.chat_id,user or 00000000)
end
end
if tonumber(msg.reply_to_message_id) == 0 then
...
if matches and (matches:match('^setowner (%d+)') or matches:match('^مالک (%d+)')) and is_JoinChannel(msg) then
...
if matches and (matches:match('^setowner @(.*)') or matches:match('^مالک @(.*)')) and is_JoinChannel(msg) then
...
if matches and (matches:match('^remowner (%d+)') or matches:match('^حذف مالک (%d+)')) and is_JoinChannel(msg) then
...
if matches and (matches:match('^remowner @(.*)') or matches:match('^حذف مالک @(.*)')) and is_JoinChannel(msg) then
...
-------------------------
if (matches == 'sup' or matches == 'دریافت پشتیبانی') then
sendText(msg.chat_id,msg.id,  'لینک گروه پشتیبانی:\n'..check_markdown(LinkSuppoRt)..'', 'md')
end
end
----
if is_Mod(msg)  then
...
if status[2] == 'del' or status[2] == 'پاکسازی' then
...
if matches and (matches:match('^warnmax (%d+)') or matches:match('^تنظیم تعداد اخطار (%d+)')) and is_JoinChannel(msg) then
local num = matches:match('^warnmax (%d+)') or matches:match('^تنظیم تعداد اخطار (%d+)')
if tonumber(num) < 2 then
sendText(msg.chat_id, msg.id, '💥️ عددی بزرگتر از *۲* بکار ببرید','md')
else
...
if matches and (matches:match('^rmsg (%d+)$') or matches:match('^پاکسازی (%d+)$')) and is_JoinChannel(msg) then
local limit = tonumber(matches:match('^rmsg (%d+)$')) or tonumber(matches:match('^پاکسازی (%d+)$'))
...
if (matches == 'panel' or matches == 'پنل') and is_JoinChannel(msg) then
...
if (matches == 'راهنما' or matches == 'help') and is_JoinChannel(msg) then
function GetPanel(MaTaDoR,Company)
if Company.results and Company.results[0] then
sendInline(msg.chat_id,msg.id, 0, 1, Company.inline_query_id,Company.results[0].id)
else
...
local Text = '🔐 *تنظیمات قفلی* `('..Company.title..')` *:*\n\n┑ ️*لینک :* '..Link..'\n┥ ️*ویرایش :* '..edit..'\n┥ *تگ :* '..tag..'\n┥ ️*هشتگ :* '..hashtag..'\n┥ ️*اینلاین :* '..inline..'\n┥ ️*فیلم‌سلفی :* '..video_note..'\n┥ *سنجاق :* '..pin..'\n┥ ️*استیکر :* '..sticker..'\n┥ ️*فوروارد :* '..fwd..'\n┥ ️*فارسی :* '..arabic..'\n┥ ️*انگلیسی :* '..en..'\n┥ ️*سرویس‌تلگرام :* '..tg..'\n┙ ️*فونت :* '..mak..'\n\n〘     🗯 _تنظیمات رسانه_      〙\n\n┑ ️*رسانه‌ها :* '..caption..'\n┥ ️*عکس :* '..photo..'\n┥ ️*آهنگ :* '..music..'\n┥ *ویس :* '..voice..'\n┥ ️*فایل :* '..document..'\n┥ ️*فیلم :* '..video..'\n┥ ️*بازی :* '..game..'\n┥ ️*موقعیت :* '..location..'\n┥ ️*گیف :* '..gif..'\n┥ ️*مخاطب :* '..contact..'\n┙ ️*متن :* '..txts..'\n\n〘     ♻️ _تنظیمات بیشتر_      〙\n\n┑ ️*گروه :* '..muteall..'\n┥ *ریپلای :* '..rep..'\n┥ ️*ربات :* '..bot..'\n┥ ️*دستورات :* '..cmd..'\n┥ *اسپم :* '..spam..'\n┥ ️*فلود :* '..flood..'\n┙ ️*خوشآمد‌گوی :* '..welcome..'\n\n┑ ️*قفل خودکار  :* '..auto..'\n┥ ️*زمان شروع :* '..start..'\n┙ ️*زمان پایان :* '..stop..'\n\n┑ ️*موقعیت پیام‌مکرر :* '..Status..'\n┥ ️*زمان‌برسی پیام‌مکرر :* '..TIME_CHECK..'\n┥ ️*حداکثر پیام مکرر :* '..NUM_MSG_MAX..'\n┥ *حداکثر اسپم :* '..NUM_CH_MAX..'\n┙ *حداکثر اخطار :* '..warn..'\n\n┥ 📅*تاریخ انقضا :* '..EXPIRE..'\n\n┑ 💻*سازنده ربات :* '..check_markdown(UserSudo)..'\n┙ 🌐*کانال ما :* '..check_markdown(Channel)..'\n'
sendText(msg.chat_id, msg.id, Text, 'md')
end
GetChat(msg.chat_id,GetName)
end
...
if (matches == 'welc enable' or matches == 'خوشآمد فعال') and is_JoinChannel(msg) then
if redis:get('Welcome:'..msg.chat_id) == 'enable' then
...
if (matches == 'welc disable' or matches == 'خوشآمد غیرفعال') and is_JoinChannel(msg) then
...
-----------------------------------------------Locks------------------------------------------------
if matches == 'قفل دستورات' and is_JoinChannel(msg) then
if redis:get('Lock:Cmd'..msg.chat_id) then
...
if redis:get('Lock:Cmd'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock cmd is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock cmd Has Been Enable.`' , 'md')
redis:set('Lock:Cmd'..msg.chat_id,true)
end
end
if matches == 'بازکردن دستورات' and is_JoinChannel(msg) then
if redis:get('Lock:Cmd'..msg.chat_id) then
...
if redis:get('Lock:Cmd'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock cmd Has Been Disable.`' , 'md')
redis:del('Lock:Cmd'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock cmd is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل فونت' and is_JoinChannel(msg) then
...
if redis:get('Lock:Markdown:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock MarkDown is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock MarkDown Has Been Enable.`' , 'md')
redis:set('Lock:Markdown:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فونت' and is_JoinChannel(msg) then
if redis:get('Lock:Markdown:'..msg.chat_id) then
...
if redis:get('Lock:Markdown:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock MarkDown Has Been Disable.`' , 'md')
redis:del('Lock:Markdown:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock MarkDown is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل عنوان' and is_JoinChannel(msg) then
if redis:get('Mute:Caption:'..msg.chat_id) then
...
if redis:get('Mute:Caption:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Caption is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Caption Has Been Enable.`' , 'md')
redis:set('Mute:Caption:'..msg.chat_id,true)
end
end
if matches == 'بازکردن عنوان' and is_JoinChannel(msg) then
if redis:get('Mute:Caption:'..msg.chat_id) then
...
if matches == 'قفل ریپلای' and is_JoinChannel(msg) then
...
if redis:get('Mute:Reply:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Reply Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Reply Has Been Enable.`' , 'md')
redis:set('Mute:Reply:'..msg.chat_id,true)
end
end
if matches == 'بازکردن ریپلای' and is_JoinChannel(msg) then
if redis:get('Mute:Reply:'..msg.chat_id) then
...
if redis:get('Mute:Reply:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Reply Has Been Disable.`' , 'md')
redis:del('Mute:Reply:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Reply Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل لینک' and is_JoinChannel(msg) then
...
if redis:get('Lock:Link'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Link Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Link Has Been Enable.`' , 'md')
redis:set('Lock:Link'..msg.chat_id,true)
end
end
if matches == 'بازکردن لینک' and is_JoinChannel(msg) then
if redis:get('Lock:Link'..msg.chat_id) then
...
if redis:get('Lock:Link'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Link Has Been Disabel.`' , 'md')
redis:del('Lock:Link'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Lɪɴᴋ Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل تگ' and is_JoinChannel(msg) then
...
if redis:get('Lock:Tag:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Tag Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Tag Has Been Enable.`' , 'md')
redis:set('Lock:Tag:'..msg.chat_id,true)
end
end
if matches == 'بازکردن تگ' and is_JoinChannel(msg) then
if redis:get('Lock:Tag:'..msg.chat_id) then
...
if redis:get('Lock:Tag:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Tag Has Been Disable.`' , 'md')
redis:del('Lock:Tag:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Tag Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل هشتگ' and is_JoinChannel(msg) then
...
if redis:get('Lock:HashTag:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Hashtag Is AlReady Enabled.`' , 'md')
else 
sendText(msg.chat_id, msg.id, '`💥️ Lock Hashtag Has Been Enable.`' , 'md')
redis:set('Lock:HashTag:'..msg.chat_id,true)
end
end
if matches == 'بازکردن هشتگ' and is_JoinChannel(msg) then
if redis:get('Lock:HashTag:'..msg.chat_id) then
...
if redis:get('Lock:HashTag:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Hashtag Has Been Disable.`' , 'md')
redis:del('Lock:HashTag:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Hashtag Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل سلفی' and is_JoinChannel(msg) then
...
if redis:get('Lock:Video_note:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock VideoNote Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock VideoNote Has Been Enable.`' , 'md')
redis:set('Lock:Video_note:'..msg.chat_id,true)
end
end
if matches == 'بازکردن سلفی' and is_JoinChannel(msg) then
if redis:get('Lock:Video_note:'..msg.chat_id) then
...
if redis:get('Lock:Video_note:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock VideoNote Has Been Disable.`' , 'md')
redis:del('Lock:Video_note:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock VideoNote Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل اسپم' and is_JoinChannel(msg) then
if redis:get('Spam:Lock:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل اسپم از قبل فعال بود.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ قفل اسپم فعال شد.`' , 'md')
redis:set('Spam:Lock:'..msg.chat_id,true)
end
end
if matches == 'lock spam' then
if redis:get('Spam:Lock:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Spam Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Spam Has Been Enable.`' , 'md')
redis:set('Spam:Lock:'..msg.chat_id,true)
end
end
if matches == 'بازکردن اسپم' and is_JoinChannel(msg) then
if redis:get('Spam:Lock:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل اسپم غیرفعال شد.`' , 'md')
redis:del('Spam:Lock:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ قفل اسپم از قبل غیرفعال بود.`' , 'md')
end
end
if matches == 'unlock spam' then
if redis:get('Spam:Lock:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Spam Has Been Disable.`' , 'md')
redis:del('Spam:Lock:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Spam Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل اینلاین' and is_JoinChannel(msg) then
...
if redis:get('Lock:Inline:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Inline Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Inline Has Been Enable.`' , 'md')
redis:set('Lock:Inline:'..msg.chat_id,true)
end
end
if matches == 'بازکردن اینلاین' and is_JoinChannel(msg) then
if redis:get('Lock:Inline:'..msg.chat_id) then
...
if matches == 'unlock inline' and is_JoinChannel(msg) then
if redis:get('Lock:Inline:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Inline Has Been Disable.`' , 'md')
redis:del('Lock:Inline:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Inline Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل فلود' and is_JoinChannel(msg) then
if redis:get('Lock:Flood:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل فلود از قبل فعال بود.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ قفل فلود فعال شد.`' , 'md')
redis:set('Lock:Flood:'..msg.chat_id,true)
end
end
if matches == 'lock flood' and is_JoinChannel(msg) then
if redis:get('Lock:Flood:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Flood Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Flood Has Been Enable.`' , 'md')
redis:set('Lock:Flood:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فلود' and is_JoinChannel(msg) then
if redis:get('Lock:Flood:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل فلود غیرفعال شد.`' , 'md')
redis:del('Lock:Flood:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ قفل فلود غیرفعال بود.`' , 'md')
end
end
if matches == 'unlock flood' and is_JoinChannel(msg) then
if redis:get('Lock:Flood:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Flood Has Been Disable.`' , 'md')
redis:del('Lock:Flood:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Flood Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل فوروارد' and is_JoinChannel(msg) then
if redis:get('Lock:Forward:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل فوروارد از قبل فعال بود.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ قفل فوروارد فعال شد.`' , 'md')
redis:set('Lock:Forward:'..msg.chat_id,true)
end
end
if matches == 'lock forward' and is_JoinChannel(msg) then
if redis:get('Lock:Forward:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Forward Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Forward Has Been Enable.`' , 'md')
redis:set('Lock:Forward:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فوروارد' and is_JoinChannel(msg) then
if redis:get('Lock:Forward:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ قفل فوروارد غیرفعال شد.`' , 'md')
redis:del('Lock:Forward:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ قفل فوروارد از قبل غیرفعال بود.`' , 'md')
end
end
if matches == 'unlock forward' and is_JoinChannel(msg) then
if redis:get('Lock:Forward:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Forward Has Been Disable.`' , 'md')
redis:del('Lock:Forward:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Forward Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل فارسی' and is_JoinChannel(msg) then
...
if matches == 'lock farsi' and is_JoinChannel(msg) then
if redis:get('Lock:Arabic:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Parsian Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock parsian Has Been Enable.`' , 'md')
redis:set('Lock:Arabic:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فارسی' and is_JoinChannel(msg) then
if redis:get('Lock:Arabic:'..msg.chat_id) then
...
if matches == 'unlock farsi' and is_JoinChannel(msg) then
if redis:get('Lock:Arabic:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Parsian Has Been Disable.`' , 'md')
redis:del('Lock:Arabic:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Parsian Is AlReady Disabled.`' , 'md')
end
end
if matches == 'قفل ربات' and is_JoinChannel(msg) then
...
if redis:get('Lock:Bot:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Bots Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Bots Has Been Enable.`' , 'md')
redis:set('Lock:Bot:'..msg.chat_id,true)
end
end
if matches == 'بازکردن ربات' and is_JoinChannel(msg) then
if redis:get('Lock:Bot:'..msg.chat_id) then
...
if redis:get('Lock:Bot:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Bots Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Lock:Bot:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Bots Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل ویرایش' and is_JoinChannel(msg) then
...
if redis:get('Lock:Edit'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Eᴅɪᴛ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Eᴅɪᴛ Has Been Enable.`' , 'md')
redis:set('Lock:Edit'..msg.chat_id,true)
end
end
if matches == 'بازکردن ویرایش' and is_JoinChannel(msg) then
if redis:get('Lock:Edit'..msg.chat_id) then
...
if redis:get('Lock:Edit'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Eᴅɪᴛ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Lock:Edit'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Eᴅɪᴛ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل انگلیسی' and is_JoinChannel(msg) then
...
if redis:get('Lock:English:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Eɴɢʟɪsʜ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Eɴɢʟɪsʜ Has Been Enable.`' , 'md')
redis:set('Lock:English:'..msg.chat_id,true)
end
end
if matches == 'بازکردن انگلیسی' and is_JoinChannel(msg) then
if redis:get('Lock:English:'..msg.chat_id) then
...
if redis:get('Lock:English:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Eɴɢʟɪsʜ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Lock:English:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Eɴɢʟɪsʜ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل سرویس' and is_JoinChannel(msg) then
...
if redis:get('Lock:TGservise:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock TGSᴇʀᴠɪᴄᴇ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock TGSᴇʀᴠɪᴄᴇ Has Been Enable.`' , 'md')
redis:set('Lock:TGservise:'..msg.chat_id,true)
end
end
if matches == 'بازکردن سرویس' and is_JoinChannel(msg) then
if redis:get('Lock:TGservise:'..msg.chat_id) then
...
if redis:get('Lock:TGservise:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock TGSᴇʀᴠɪᴄᴇ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Lock:TGservise:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock TGSᴇʀᴠɪᴄᴇ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل استیکر' and is_JoinChannel(msg) then
...
if redis:get('Lock:Sticker:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Sᴛɪᴄᴋᴇʀ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Sᴛɪᴄᴋᴇʀ Has Been Enable.`' , 'md')
redis:set('Lock:Sticker:'..msg.chat_id,true)
end
end
if matches == 'بازکردن استیکر' and is_JoinChannel(msg) then
if redis:get('Lock:Sticker:'..msg.chat_id) then
...
if redis:get('Lock:Sticker:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Sᴛɪᴄᴋᴇʀ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Lock:Sticker:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Sᴛɪᴄᴋᴇʀ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل متن' and is_JoinChannel(msg) then
if redis:get('Mute:Text:'..msg.chat_id) then
...
if redis:get('Mute:Text:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Tᴇxᴛ IS AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Tᴇxᴛ Has Been Enable.`' , 'md')
redis:set('Mute:Text:'..msg.chat_id,true)
end
end
if matches == 'بازکردن متن' and is_JoinChannel(msg) then
if redis:get('Mute:Text:'..msg.chat_id) then
...
if redis:get('Mute:Text:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Tᴇxᴛ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Text:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Tᴇxᴛ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل مخاطب' and is_JoinChannel(msg) then
if redis:get('Mute:Contact:'..msg.chat_id) then
...
if redis:get('Mute:Contact:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Cᴏɴᴛᴀᴄᴛ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Cᴏɴᴛᴀᴄᴛ Has Been Enable.`' , 'md')
redis:set('Mute:Contact:'..msg.chat_id,true)
end
end
if matches == 'بازکردن مخاطب' and is_JoinChannel(msg) then
if redis:get('Mute:Contact:'..msg.chat_id) then
...
if redis:get('Mute:Contact:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Cᴏɴᴛᴀᴄᴛ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Contact:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Cᴏɴᴛᴀᴄᴛ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل فایل' and is_JoinChannel(msg) then
if redis:get('Mute:Document:'..msg.chat_id) then
...
if redis:get('Mute:Document:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Dᴏᴄᴜᴍᴇɴᴛ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Dᴏᴄᴜᴍᴇɴᴛ Has Been Enable.`' , 'md')
redis:set('Mute:Document:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فایل' and is_JoinChannel(msg) then
if redis:get('Mute:Document:'..msg.chat_id) then
...
if redis:get('Mute:Document:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Dᴏᴄᴜᴍᴇɴᴛ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Document:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Dᴏᴄᴜᴍᴇɴᴛ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل موقعیت' and is_JoinChannel(msg) then
if redis:get('Mute:Location:'..msg.chat_id) then
...
if redis:get('Mute:Location:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Lᴏᴄᴀᴛɪᴏɴ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Lᴏᴄᴀᴛɪᴏɴ Has Been Enable.`' , 'md')
redis:set('Mute:Location:'..msg.chat_id,true)
end
end
if matches == 'بازکردن موقعیت' and is_JoinChannel(msg) then
if redis:get('Mute:Location:'..msg.chat_id) then
...
if redis:get('Mute:Location:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Lᴏᴄᴀᴛɪᴏɴ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Location:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Lᴏᴄᴀᴛɪᴏɴ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل ویس' and is_JoinChannel(msg) then
if redis:get('Mute:Voice:'..msg.chat_id) then
...
if redis:get('Mute:Voice:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Vᴏɪᴄᴇ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Vᴏɪᴄᴇ Has Been Enable.`' , 'md')
redis:set('Mute:Voice:'..msg.chat_id,true)
end
end
if matches == 'بازکردن ویس' and is_JoinChannel(msg) then
if redis:get('Mute:Voice:'..msg.chat_id) then
...
if redis:get('Mute:Voice:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Vᴏɪᴄᴇ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Voice:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Vᴏɪᴄᴇ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل عکس' and is_JoinChannel(msg) then
if redis:get('Mute:Photo:'..msg.chat_id) then
...
if redis:get('Mute:Photo:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Pʜᴏᴛᴏ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Pʜᴏᴛᴏ Has Been Enable.`' , 'md')
redis:set('Mute:Photo:'..msg.chat_id,true)
end
end
if matches == 'بازکردن عکس' and is_JoinChannel(msg) then
if redis:get('Mute:Photo:'..msg.chat_id) then
...
if redis:get('Mute:Photo:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Pʜᴏᴛᴏ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Photo:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Pʜᴏᴛᴏ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل بازی' and is_JoinChannel(msg) then
if redis:get('Mute:Game:'..msg.chat_id) then
...
if redis:get('Mute:Game:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Gᴀᴍᴇ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Gᴀᴍᴇ Has Been Enable.`' , 'md')
redis:set('Mute:Game:'..msg.chat_id,true)
end
end
if matches == 'بازکردن بازی' and is_JoinChannel(msg) then
if redis:get('Mute:Game:'..msg.chat_id) then
...
if redis:get('Mute:Game:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Gᴀᴍᴇ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Game:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Gᴀᴍᴇ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل فیلم' and is_JoinChannel(msg) then
if redis:get('Mute:Video:'..msg.chat_id) then
...
if redis:get('Mute:Video:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Vɪᴅᴇᴏ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Vɪᴅᴇᴏ Has Been Enable.`' , 'md')
redis:set('Mute:Video:'..msg.chat_id,true)
end
end
if matches == 'بازکردن فیلم' and is_JoinChannel(msg) then
if redis:get('Mute:Video:'..msg.chat_id) then
...
if redis:get('Mute:Video:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Vɪᴅᴇᴏ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Video:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Vɪᴅᴇᴏ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل آهنگ' and is_JoinChannel(msg) then
if redis:get('Mute:Music:'..msg.chat_id) then
...
if redis:get('Mute:Music:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Mᴜsɪᴄ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Mᴜsɪᴄ Has Been Enable.`' , 'md')
redis:set('Mute:Music:'..msg.chat_id,true)
end
end
if matches == 'بازکردن آهنگ' and is_JoinChannel(msg) then
if redis:get('Mute:Music:'..msg.chat_id) then
...
if matches == 'unlock music' and is_JoinChannel(msg) then
if redis:get('Mute:Music:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Mᴜsɪᴄ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Music:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Mᴜsɪᴄ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
if matches == 'قفل گیف' and is_JoinChannel(msg) then
if redis:get('Mute:Gif:'..msg.chat_id) then
...
if matches == 'lock gif' and is_JoinChannel(msg) then
if redis:get('Mute:Gif:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Gɪғ Is AlReady Enabled.`' , 'md')
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Gɪғ Has Been Enable.`' , 'md')
redis:set('Mute:Gif:'..msg.chat_id,true)
end
end
if matches == 'بازکردن گیف' and is_JoinChannel(msg) then
if redis:get('Mute:Gif:'..msg.chat_id) then
...
if matches == 'unlock gif' and is_JoinChannel(msg) then
if redis:get('Mute:Gif:'..msg.chat_id) then
sendText(msg.chat_id, msg.id, '`💥️ Lock Gɪғ Has Been Dɪsᴀʙʟᴇ.`' , 'md')
redis:del('Mute:Gif:'..msg.chat_id)
else
sendText(msg.chat_id, msg.id, '`💥️ Lock Gɪғ Is AlReady Dɪsᴀʙʟᴇᴅ.`' , 'md')
end
end
----------------------------------------------------------------------------------
if matches1 and (matches1:match('^[Ss]etlink (.*)') or matches1:match('^تنظیم لینک (.*)')) and is_JoinChannel(msg) then
...
if matches and (matches:match('^getwebpage (https://)(.*)') or matches:match('^سایت (https://)(.*)')) and is_JoinChannel(msg) then
...
if (matches == 'saman'  or matches == 'سامان') and is_sudo(msg)  then
sendText(msg.chat_id, msg.id,'سازنده ربات😢', 'html')
end 
if matches == 'پیام رسان' and is_sudo(msg) then
sendText(msg.chat_id, msg.id,'✦ ربات پیام رسان برای ارتباط سودو من(:  '..PvUserSudo..'', 'html')
end
...
local rankpro = {''..Bot..'با من چکار داری؟','بگو '..Bot..'رباتو درد خستم کردی😩','بلی '..Bot..'','فدای چشمات چیه بگو '..Bot..''}
sendText(msg.chat_id, msg.id,rankpro[math.random(#rankpro)], 'md')
else
local rank = {'جونم '..check_markdown(CompanyName)..'','بگو '..check_markdown(CompanyName)..'','بلی '..check_markdown(CompanyName)..'','فدای چشمات چیه بگو '..check_markdown(CompanyName)..''}
...
sendSticker(msg.chat_id,msg.id,'./logs/Ping.webp')
end
-------------------------------
if (matches == 'انلاینی' or matches == 'online') then
local function GetName(MaTaDoR, Company)
if Company.first_name then
CompanyName = ec_name(Company.first_name)
else  
...
sendText(msg.chat_id, msg.id,'آره آنلاینم مگه (سامان) میزاره آفلاین بشم '..check_markdown(CompanyName)..' جون 😍', 'html')
end
GetUser(msg.sender_user_id,GetName)
end
if matches == 'راهنمای کاربران' and is_JoinChannel(msg) then
sendText(msg.chat_id,msg.id,'راهنما کاربران (مگا پلاس)💥\nMembarHelp (MeGaPlus)\n\n🔹 Id️\n🔸 ایدی️\n✮ نمایش آیدی کاربر️\n▪️➖▪️➖️➖▪️➖▪️\n🔹 Me️\n🔸 اطلاعات من️\n✮ دریافت اطلاعات خود️\n▪️➖▪️➖️➖▪️➖▪️\n🔹 Whois [id]️\n🔸 اطلاعات [ایدی]️\n✮ دریافت اطلاعات فرد️\n▪️➖▪️➖➖▪️➖▪️\n🔹 Getpro [num]️\n🔸 پروفایل [عدد]️\n✮ دریافت پروفایل خود️\n▪️➖▪️➖➖▪️➖▪️️️\n🔹 Groupinfo️\n🔸 اطلاعات گروه️\n✮ دریافت اطلاعات گروه️\n▪️➖▪️➖️➖▪️➖▪️\n🔹 Link️\n🔸 لینک️\n✮ دریافت لینک گروه️\n▪️➖▪️➖➖▪️➖▪️️\n🔹 Rules️\n🔸 قوانین️\n✮ دریافت قوانین گروه️\n▪️➖▪️➖️➖▪️➖▪️️️\n🔹 Games️\n🔸 ارسال بازی️\n✮ ارسال بازی برای کاربران️\n▪️➖▪️➖➖▪️➖▪️️️\n🔹 Time️\n🔸 زمان️\n✮ نمایش ساعت فعلی️\n▪️➖▪️➖➖▪️➖▪️\n🔹 Online🔸 انلاینی️\n✮ با خبر شدن از انلاینی ربات\n▪️➖▪️➖▪️➖▪️➖▪️\n🌐 کانال بیوگرافے :\n'..check_markdown(UserSudo)..'\n🌐 کانال پشتیبانے ربات:\n🌐 '..check_markdown(UserSudo)..'','md')
end
--------------------------------
end
------MaTaDoR Company---------.
